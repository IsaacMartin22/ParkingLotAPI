...
- Domain hierarchy is `ParkingLot -> Floor -> Section -> ParkingSpace` (`src/main/java/com/example/apiservice/dbentity`).
...
- Operationally common writes are `ParkingSpace`; `Floor` and `Section` mutation endpoints exist but are called out as rare in `index.html`.
...
- If you change `ParkingSpaceService.save`, preserve SSE publication semantics for floor-scoped updates.
